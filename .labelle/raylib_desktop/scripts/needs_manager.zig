...
const navigation_orchestrator = @import("navigation_orchestrator.zig");
...
pub fn update(game: *Game, scene: *Scene, dt: f32) void {
    ...
    for (ds_ids[0..ds_count]) |wid| {
        ...
        if (registry.tryGet(Position, bed_entity)) |bed_pos| {
            // Cancel any active navigation/movement before starting new one
            navigation_orchestrator.cancelNavigation(registry, worker_entity, wid);
